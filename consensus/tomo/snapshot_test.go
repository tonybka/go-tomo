...
func (r *testerChainReader) Config() *params.ChainConfig                 { return params.AllTomoProtocolChanges }
...
func TestVoting(t *testing.T) {
	...
	for i, tt := range tests {
		...
		// Pass all the headers through tomo and ensure tallying succeeds
		head := headers[len(headers)-1]

		snap, err := New(&params.TomoConfig{Epoch: tt.epoch}, db).snapshot(&testerChainReader{db: db}, head.Number.Uint64(), head.Hash(), headers)
		if err != nil {
